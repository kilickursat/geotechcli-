...
- Text/agent reasoning defaults to `glm-5.2`; vision defaults to `glm-5v-turbo`; PDF/table layout extraction uses hosted `glm-ocr` through Z.ai.
...
| `geotech fem run` | Human-invoked experimental execution of a reviewed FEM `analysis_case.json` through deterministic preview backends, including optional `--backend nonlinear-column` and `--backend biot-up`; requires `--experimental --reviewed` |
...
| `geotech fem demo biot` | Experimental deterministic plane-strain Biot u-p seepage/consolidation WebGL preview |
