...
Agents and swarm runs now see FEM through deterministic routing tools, not prompt-only instructions. `geotech fem agent` is a narrower LLM brain for FEM planning: it may only list FEM capabilities, prepare review-gated case drafts, and validate FEM cases. With `--workspace`, it receives FEM-only GroundModel readiness context and the same evidence-prefill contract as `geotech fem draft`. It cannot run solvers, write WebGL artifacts, or invent FEM displacement/reaction values. Blocked validation is returned as inspectable review data so reviewer agents can reject unsafe cases with specific blocker codes. Draft and planned routes include shaft deformation, tunnel volume-loss settlement, and pile-group interaction as contract-aware future targets until deterministic engines are added.
