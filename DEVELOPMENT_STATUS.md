...
- Use OCR-optimized margin trimming, projection-profile deskew, normalized raster sizing, smarter table/log crop candidates, normalized region crop assets, and region/page quality scoring as the measurable PDF/image preprocessing pass.
- Run detected preprocessing region crops through vision OCR before generic dense-page chunking so table/log panels get a tighter OCR target without changing the provider-neutral evidence contract.
- Keep page evidence cache keys mode-aware with `GEOTECHCLI_PREPROCESSING_MODE=none|ocr-optimized`, and compare preprocessing modes in benchmark output through quality, deskew, region, and crop-asset deltas.
...
- Add deterministic `geotech signal analyze` V0 for monitoring/time-series CSV/TSV/XLSX inputs before any agent interpretation layer consumes settlement, piezometer, inclinometer, vibration, or load-test summaries; threshold flags stay opt-in because units vary by instrument and project convention.
...
- Added `npm run smoke:signal-analysis` and `npm run benchmark:signal-analysis` to generate synthetic settlement, piezometer, and inclinometer fixtures, verify persisted signal artifact indexes and analysis JSON, assert direct threshold/rate/missing-interval metrics, assert empty `model_calls.jsonl`, and write comparison JSON plus an SVG summary.
