...
In-progress after v0.4.90: The internal corpus now includes a cached mixed digital/scanned PDF fixture and a dedicated `GEOTECHCLI_BENCHMARK_MIXED_DIGITAL_SCANNED_PDF` private real-fixture env path, so native-text plus scanned-page reports are measured through the same provider, preprocessing, cache-reuse, traceability, GroundModel-readiness, trend/dashboard, and redaction contract as the other fixture classes; the corpus trend JSON and HTML dashboard are now core-built, core-validated, and summary-only without raw benchmark JSON, report text, model IDs, private paths, or tokens. The preprocessing fixture benchmark now has a typed report/trend artifact contract, all-mode comparison deltas, path/secret safety metadata, and path-safe history/trend JSON plus a trend HTML dashboard, so borehole-log, CPT, lab, mixed-scanned, mixed digital/scanned, and malformed scanned PDF fixtures fail closed when region-v2 crop assets, mode comparisons, raw fixture filenames, asset hashes, or redaction guardrails regress. BYOK live smoke reports now have a report-level contract validator, persisted `contractValidation` metadata, redaction pass, OpenAI-compatible model env fallback coverage, and summary-only history/trend artifacts so hosted-beta, OpenAI-compatible, OpenRouter/free, and local/HF-compatible runs remain comparable against the same preprocessed page-evidence contract without persisting private paths, token-shaped values, image/PDF task claims, raw prompts, source-evidence snippets, provider responses, or model-specific evidence assumptions. GroundModel-derived planned FEM routes now have a dedicated acceptance fixture, runtime execution-boundary validator, and benchmark/corpus guardrails. The current slice keeps shaft, pile-group, slope/embankment, retaining-wall/excavation-support, seepage/groundwater, and staged settlement/consolidation routes contract-only, preserves route-specific review gates, blocks agent solver/WebGL/result-manifest actions, proves evidence-prefill for material and groundwater values without exposing runnable cases, redacts local absolute evidence source paths, rejects raw prompt/response/source-evidence/model payload keys and result-manifest/solver/WebGL payloads, and makes benchmark comparison fail when contract-only routes lose blocked-until metadata, disallowed agent actions, review gates, or safe draft-only boundaries. Standards-profile validation now has a fixture-backed runtime contract guard for Eurocode 7, AASHTO, IS, BS, and ASTM readiness output so assumptions, blocker codes, safety-factor context, and source references stay deterministic while run commands, solver/FEM fields, design-result payloads, raw prompt/response/source-evidence/model payload keys, private paths, token-shaped values, and unregistered source references fail closed. GroundModel-to-calculation input drafts now also have a fixture-backed contract guard for bearing, settlement, pile, liquefaction, and slope workflows so draft outputs stay opt-in, retain missing-user-input review gates, preserve source references and evidence IDs, reject FEM/run/case-output execution leakage, raw prompt/response/source-evidence/model payload keys, and keep pile unit-weight evidence traceable when it is used in pile inputs. Deterministic signal-analysis outputs now have a fixture-backed contract for settlement, piezometer, inclinometer, vibration, and load-test profiles; the contract requires trend/rate/series structures, generic threshold-profile review gates, path-safe source labels, and no model/LLM metadata, private paths, or token-shaped values. Signal benchmark comparison and trend artifacts now also have a core contract validator, persisted `contractValidation` metadata, and fail-closed checks for all five synthetic instrument classes, zero model calls, direct settlement threshold-profile metrics, direct HTML plot payloads, and path/secret-safe benchmark history.

v0.4.90 adds a provider-neutral BYOK evidence-contract benchmark so hosted-beta, OpenAI-compatible, OpenRouter/free, and local/HF-compatible routes are tested against the same preprocessed page evidence, source-evidence citation rules, review gates, and no direct image/PDF inspection claims, with structured JSON report output from `npm run smoke:byok`. v0.4.89 adds machine-readable path-safety checks to the internal geotechnical corpus benchmark so fixture and benchmark inputs fail closed on local absolute paths or secret-shaped values without echoing private paths or BYOK tokens into JSON/HTML artifacts. v0.4.88 hardens the internal geotechnical corpus benchmark with explicit hosted-beta, OpenAI-compatible BYOK, OpenRouter/free, and local/HF-compatible provider profiles, fail-closed text-evidence routing checks, and corpus confidence component summaries for extraction, traceability, corroboration, readiness, and page-evidence trust. v0.4.87 fixes the trusted-publishing release build by giving the web package a temporary npm lockfile during Next.js build so the Node 24 publish job selects npm during SWC registry fallback instead of Yarn. v0.4.86 adds path-safe signal benchmark history and trend outputs, including `signal-history.json`, `signal-trend.json`, and `signal-trend.html`, so deterministic monitoring benchmarks can be compared over time without storing raw monitoring files or private local paths. v0.4.85 adds opt-in deterministic signal threshold profiles for settlement, piezometer, inclinometer, vibration PPV, and load-test monitoring data, with review gates that require project-specific trigger levels before engineering acceptance, plus path-safe signal benchmark artifacts across all five instrument classes. v0.4.84 extends opt-in GroundModel calculation input drafts with structured source references, grouped source pages, evidence-derived draft confidence, and explicit review gates, and fixes the broader preprocessing v2 fixture test timeout seen in full core CI. v0.4.83 adds deterministic standards-profile validation for Eurocode 7, AASHTO, IS, BS, and ASTM readiness, including blocker codes, required assumptions, safety-factor context, and source-reference anchors wired into GroundModel verification without auto-running design calculations. v0.4.82 adds a deterministic no-model preprocessing benchmark for the broader fixture set. The benchmark compares `none`, `ocr-optimized`, and `region-v2` across borehole/table, CPT table, lab table, mixed scanned report, mixed digital/scanned PDF, and malformed scanned fixtures, then writes JSON, HTML, and SVG summaries with latency, quality, crop-asset, region-count, and region-quality deltas while failing on local path or secret leaks. v0.4.81 started the broader PDF/image preprocessing v2 fixture expansion with deterministic scanned CPT table, lab table, and mixed scanned-report PDFs, plus preprocessing-only tests that prove `region-v2` crop detection, normalized crop assets, and region quality across those page shapes without hosted model calls. v0.4.80 closed the first `region-v2` proof gap by adding a deterministic in-repo image-only borehole/table PDF fixture and making `region-v2` a real corpus acceptance gate. The fixture proves rendered PDF evidence can produce preprocessing region coverage, normalized crop assets, region quality scores, and region-first OCR inputs, while the corpus validator now fails if `region-v2` returns zero regions/assets or regresses traceability below `none` and `ocr-optimized`. v0.4.79 added the underlying `region-v2` PDF/image preprocessing machinery: fine-pass deskew metadata, higher-resolution normalized pages, smarter table and borehole/log strip crop candidates, persisted normalized region assets, region quality scoring, and region-first OCR routing before full-page vision OCR. The corpus runner compares `none`, `ocr-optimized`, and `region-v2` by default across hosted-beta and BYOK text-evidence profiles so preprocessing changes are judged by evidence coverage, region quality, cache reuse, hosted-call estimates, traceability, GroundModel readiness, and review gates rather than visual impression alone. The integrated ingest report UI and GroundModel evidence path continue preserving every recovered report borehole in the evidence, strip-log, field, map, and A-A section views, while project-aware agents keep deterministic `calculation-readiness` routing for bearing, settlement, pile, liquefaction, slope, and FEM draft workflows before any LLM review. The LLM boundary remains tight: hosted GLM and future BYOK models can propose, plan, draft, validate, and review project workflows and FEM cases, while GeotechCLI deterministic contracts own calculations, FEM case data, visualization specs, confidence, and persisted artifacts.
...
- Keep BYOK live smoke benchmarks on the same evidence contract: compact preprocessed page evidence in, compact JSON with cited evidence IDs and review gates out, with no image/PDF claims unless the task explicitly routes through deterministic preprocessing.
- Keep corpus provider profiles explicit for hosted-beta, OpenAI-compatible BYOK, OpenRouter/free, and local/HF-compatible runs, and fail closed when a text-only/free/local profile is allowed to consume direct image or native-PDF tasks instead of preprocessed page evidence.
- Keep corpus benchmark path-safety explicit: local absolute paths and API-token-shaped values must fail acceptance and appear only as redacted findings in persisted JSON/HTML artifacts.
...
- Keep `npm run benchmark:geotech-corpus -- --real-fixtures` available for private local PDFs/images supplied by env vars such as `GEOTECHCLI_BENCHMARK_BOREHOLE_PDF`, `GEOTECHCLI_BENCHMARK_CPT_PDF`, `GEOTECHCLI_BENCHMARK_LAB_PDF`, `GEOTECHCLI_BENCHMARK_MIXED_SCANNED_PDF`, `GEOTECHCLI_BENCHMARK_MIXED_DIGITAL_SCANNED_PDF`, and `GEOTECHCLI_BENCHMARK_MALFORMED_SCANNED_PDF`; missing fixtures skip unless `--required` is set, persisted benchmark JSON redacts absolute paths, and every run writes core-validated summary-only `corpus-history.json`, `corpus-trend.json`, and `corpus-trend.html` artifacts for comparison against prior local results.
...
- Allow reviewed FEM `analysis_case.json` drafts to be run only through the human-invoked `geotech fem run ... --experimental --reviewed` path; do not expose solver/WebGL execution as an LLM or swarm tool.
...
- Block generic agent artifact/result tools from writing FEM result manifests, solver outputs, WebGL artifacts, or invented FEM analysis files; FEM artifacts must come from deterministic `geotech fem run ... --experimental --reviewed` after explicit human review.
...
- Keep FEM route recommendations current: draft creation uses `geotech fem draft ...`, reviewed analysis cases use `geotech fem run <analysis_case.json> --experimental --reviewed`, and built-in demo commands remain examples rather than agent-recommended execution paths.
