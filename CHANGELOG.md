...
- Upgraded the HTML ingest dossier with status badges, searchable review filters, a schematic ground-model cross-section, source evidence actions, and a scroll-aware human review workflow bar.
- Verified the `GeotechnicalInvestigationReport (1).pdf` ingest rerun completed with all 34 pages as cache hits, no new cache stores, 52 materials, 34 parameters, and the review-focused dossier presentation intact.
