...
Three published methods did not match their sources. Every number below changed; anyone who ran these commands on a real project should re-run them.

- **Liquefaction CRR was a mis-transcribed Boulanger & Idriss (2014) curve, and it failed unsafe.** Eq. 2.24 (`exp[N/14.1 + (N/126)² − (N/23.6)³ + (N/25.4)⁴ − 2.8]`) had been folded onto a single normalized variable with divisors 2.67/3.0/4.0, which is a different function. It over-predicted cyclic resistance by 1.2× at (N₁)₆₀cs = 10, rising to 7.4× at 25 and 39× at 30 — so the error grew with density and systematically under-flagged the marginal layers screening exists to catch. On an Mw 7.5 / 0.30 g loose-sand profile, two layers that should report FS 0.55 and 0.58 were reported as FS 1.15 "MODERATE" and FS 1.32 "LOW". Both now report SEVERE.
- The Boulanger & Idriss path is now the actual 2014 procedure rather than a mixture: density-dependent magnitude scaling (Eqs. 2.19-2.20, exactly 1.0 at Mw 7.5), the overburden correction on cyclic resistance K_σ (Eqs. 2.16-2.17, exactly 1.0 at 1 atm), the iterative C_N of Eq. 2.15b, and the magnitude-dependent Idriss (1999) r_d. The NCEER path keeps its own Youd et al. (2001) fines correction, MSF and Liao & Whitman (1986) r_d instead of borrowing B&I's — the two procedures no longer share pieces. Layer results now expose `sigmaVPrime`, `CN`, `rd`, `MSF`, `Ksigma` and `volumetricStrain` so the intermediate values are auditable.
- Post-liquefaction settlement no longer assumes a hardcoded 2 m per sample. Thickness is taken from an optional per-layer `thickness`, otherwise from the tributary distance between samples, and volumetric strain now follows the Ishihara & Yoshimine (1992) relation as parameterized by Yoshimine et al. (2006) — it depends on (N₁)₆₀cs rather than being a four-bin step function on FS alone.
- **`bearing --method hansen` was returning Vesic's numbers.** Both branches shared Vesic's `Nγ = 2(Nq+1)tanφ` and Vesic's shape factors, so the two methods were byte-identical. Each method now carries its own Nγ (Meyerhof `(Nq−1)tan(1.4φ)`, Hansen `1.5(Nq−1)tanφ`, Vesic `2(Nq+1)tanφ`) and its own shape factors (Hansen's s_q uses sinφ, Vesic's uses tanφ). Hansen and Vesic now use their own depth factors with `dγ = 1.0` and the `arctan(D/B)` form beyond D/B = 1, instead of Meyerhof's.
- **`bearing --shape square|circular` was silently ignored** unless `--length` was also supplied — all three shapes returned the strip answer. The declared shape now resolves to the B/L ratio the shape factors need. Terzaghi's Nγ surrogate is disclosed in the calculation steps rather than mis-attributed in a comment.
- **Slope stability only ever used the first soil layer, and the water table did nothing at all.** The layer-selection test reduced to a tautology that always matched layer 0, and the pore-pressure ternary was inverted so `u` was always zero — `waterTableDepth` and `saturatedUnitWeight` had no effect on any result. Slice weights are now integrated layer by layer with the saturated unit weight applied below the phreatic surface, and pore pressures are real.
- **The slope engine was not Bishop's method.** It forced `|α|` on every slice — so no slice could resist — divided by `m_α` twice, and generated slices across the full circle chord rather than between the surface entry and exit points. It is now a correct method of slices with signed base angles (`sin α = (x_c − x)/R`), proper daylighting by bisection, and the pseudo-static term applied as an inertia moment at each slice centroid. Against Taylor's chart for φ=0 it now lands within 3.7% across β = 30-60° (previously ~26% low), and a cohesionless slope returns tanφ′/tanβ to within 2%.
- **`slope --method ordinary` never ran.** Both options called the Bishop routine and only the output label changed. Fellenius is now implemented as its own non-iterative procedure and returns the conservative result it should.
- Added `engine-golden-values.test.ts`: 44 regression tests pinning these engines to published table entries and closed-form benchmarks — the Prandtl-Reissner and Terzaghi factor tables, per-method Nγ, the B&I base curve, the MSF/K_σ anchors, Taylor's chart, and the infinite-slope limit. 25 of the 44 fail against the previous implementation. The existing engine tests asserted only direction and ordering ("FS falls when PGA rises"), which every defect above satisfied, so all 830 of them passed throughout.
- The deterministic trust boundary is unchanged: the LLM interprets and orchestrates; deterministic code owns every number. This release is about those numbers being right.
