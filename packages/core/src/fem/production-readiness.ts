...
const FEATURE_REQUIREMENTS: Record<FemProductionFeature, Omit<FemProductionFeatureRequirement, 'feature'>> = {
  'nonlinear-plasticity': {
    ...
    currentCoverage: 'Deterministic nonlinear material-point coverage now includes a Mohr-Coulomb triaxial strength cap plus a Drucker-Prager/Mohr-Coulomb-compatible principal-stress return-mapping kernel with yield residual, isotropic hardening, and plastic strain state checks. A benchmark-scale mechanical-only Quad4 plane-strain Drucker-Prager evidence kernel now couples committed Gauss-point return mapping, isotropic hardening metadata, and plastic state carryover to global residual/reaction checks, and it can use audited adaptive cutback-bisection load stepping with rollback evidence for rejected increments. It is still not a production sparse solver route and has no Biot pore-pressure DOF, consistent tangent approval, staged activation, calibrated hardening soil/MCC model, or project result manifest.',
