...
describe('FEM production readiness contract', () => {
  ...
  it('exposes a scoped FEM agent tool for production-readiness blockers', async () => {
    ...
    expect(data.agentEvidenceSummary).toContain('external benchmark comparison results: 4');
    expect(data.agentEvidenceSummary).toContain('opengeosys-consolidation-staggered-biot-pressure-profile-t10');
    expect(data.agentEvidenceSummary).toContain('opengeosys-liquid-flow-h1-1dsteady-head-flux-gradient');
