...
const releases = [
  {
    ...
    changes: [
      ...
      { type: 'feat', text: 'Monthly tiers are Community Backer $10, Project Sustainer $50 (labelled Recommended, not "Most popular") and Organization Sponsor $100 — matching the live Patreon prices exactly, so the site never advertises an amount different from what a sponsor is charged' },
      { type: 'fix', text: 'Retired the Excellent Support ($50) and Diamond Supporter ($500) tiers along with hands-on collaboration time, direct maintainer access, and sponsored-feature prioritization — a donation should not buy open-ended professional obligations' },
      { type: 'fix', text: 'Removed the instruction telling sponsors to cancel their Patreon membership immediately after paying; asking someone to cancel a payment they just made undermines trust. Smoke assertions now fail if that copy, one-time wording, or a GitHub Sponsors link reappears' },
      { type: 'feat', text: 'Added a trust note across the support page and README: sponsorship is optional, never changes access, and does not purchase engineering approval, an SLA, roadmap control, or a guaranteed feature. The support ask now leads the page, with the free-vs-sponsor-supported comparison below it and a "prefer to contribute time?" panel' },
      { type: 'feat', text: 'The repository is now public at github.com/kilickursat/geotechcli- under Apache-2.0, so the open-source claim on this site resolves for everyone' },
    ],
  },
  {
    version: '0.4.133',
