...
Current scope: foundation-settlement, staged-excavation deformation, and tunnel volume-loss settlement drafts/demos, explicit draft inputs, workspace evidence prefill for foundation/excavation material and groundwater assumptions, fixed built-in demo geometry, deterministic result envelopes, validation warnings, and browser artifact export. \`geotech fem draft\` prepares editable \`analysis_case.json\` inputs without running a solver; demo commands execute only the built-in experimental previews. Raft and excavation previews use deterministic elastic screening fields; the tunnel preview uses an empirical Gaussian settlement trough from prescribed volume loss and trough-width factor. Workspace prefill can carry material stiffness, unit weight, groundwater assumptions, and evidence references into supported drafts, but geometry, loads, staging, mesh intent, volume-loss assumptions, and engineering approval still require explicit user review. The excavation preview adds staged field controls for surface settlement, horizontal displacement, and wall-deflection proxy. FEM manifests also include optional resultFields, steps, and datasets metadata so future shaft, pile-group, embankment, seepage, and slope previews can share the same validation and viewer contract. It is deliberately gated behind --experimental and is not a production FEM solver, wall-design check, tunnel lining check, face-stability check, basal-heave check, seepage analysis, or design calculation.

Agent and swarm mode see FEM through deterministic routing tools rather than prompt-only claims. geotech fem agent is a narrower LLM brain for FEM planning: it may only list FEM capabilities, prepare review-gated case drafts, and validate FEM cases. With --workspace, it receives FEM-only GroundModel readiness context and the same evidence-prefill contract as geotech fem draft for supported workspace draft routes. It cannot run solvers, write WebGL artifacts, or invent FEM displacement/reaction values. In general swarm mode, simulation-stage FEM/calculation tool outputs are threaded into reviewer prompts as deterministic tool context so RiskReviewer validation does not depend on a model restating every case detail in prose. Blocked validation is returned as inspectable review data so reviewer agents can reject unsafe cases with specific blocker codes. Shaft deformation and pile-group interaction remain draft/planned routes until deterministic engines are added. The purpose is to stabilize the provider-neutral FEM case/result contract before future GroundModel-to-calculation routing, report-derived input drafting, and agent review workflows use the same deterministic boundary.`,
  },
  {
    id: 'ingest',
