...
Agent and swarm mode see FEM through deterministic routing tools rather than prompt-only claims. geotech fem agent is a narrower LLM brain for FEM planning: it may only list FEM capabilities, prepare review-gated case drafts, and validate FEM cases. With --workspace, it receives FEM-only GroundModel readiness context and the same evidence-prefill contract as geotech fem draft. It cannot run solvers, write WebGL artifacts, or invent FEM displacement/reaction values. Blocked validation is returned as inspectable review data so reviewer agents can reject unsafe cases with specific blocker codes. Shaft deformation, tunnel volume-loss settlement, and pile-group interaction remain draft/planned routes until deterministic engines are added. The purpose is to stabilize the provider-neutral FEM case/result contract before future GroundModel-to-calculation routing, report-derived input drafting, and agent review workflows use the same deterministic boundary.`,
  },
  {
    id: 'ingest',
