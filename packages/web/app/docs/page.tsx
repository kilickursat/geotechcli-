...
Current executable preview scope: foundation-settlement, staged-excavation deformation, tunnel volume-loss settlement, and staged settlement/consolidation drafts/demos, explicit draft inputs, reviewed case-file execution through \`geotech fem run --experimental --reviewed\`, workspace and report-derived evidence prefill for material and groundwater assumptions, fixed built-in demo geometry, deterministic result envelopes, validation warnings, and browser artifact export. Shaft deformation, pile-group interaction, slope/embankment deformation, retaining-wall/excavation-support, and seepage/groundwater coupling are contract-only draft routes: they expose required evidence, missing inputs, blocker codes, and safe draft commands, but they do not create \`analysis_case.json\`, run solvers, render WebGL, claim results, or persist raw prompt/response/source-evidence/model payload keys. Local absolute evidence source paths are reduced before candidate exposure. \`geotech fem draft\` prepares editable \`analysis_case.json\` inputs only for implemented preview routes; ingest HTML reports list FEM draft candidates from the extracted GroundModel as routing guidance only; \`geotech fem run\` accepts a reviewed \`analysis_case.json\` only with explicit \`--reviewed\` acknowledgement and dispatches it through deterministic built-in preview backends; demo commands execute only the built-in experimental examples. Raft and excavation previews use deterministic elastic screening fields; the tunnel preview uses an empirical Gaussian settlement trough from prescribed volume loss and trough-width factor; the consolidation preview uses a 1D Terzaghi staged time-stepper with Mohr-Coulomb material-point strength gates. Reviewed staged consolidation cases can opt into \`--backend nonlinear-column\`, which adds a deterministic 1D vertical equilibrium solve with Drucker-Prager/Mohr-Coulomb-compatible material-point return mapping, solver load-step counts, iteration counts, force residuals, yield residuals, and plastic-strain metadata. The core engineering evidence suite also includes benchmark-scale Quad4 linear assembly, mechanical Drucker-Prager, steady seepage, and Biot u-p coupled residual kernels, including displacement and pore-pressure DOFs for Biot evidence; those kernels are not route-backed result manifests. Workspace/report prefill can carry material stiffness, unit weight, groundwater assumptions, readiness scores, and evidence references into supported drafts, but geometry, loads, staging, mesh intent, volume-loss/consolidation assumptions, and engineering approval still require explicit user review. The excavation and consolidation previews add staged field controls. FEM manifests also include optional resultFields, steps, and datasets metadata so planned routes can share the same validation and viewer contract once deterministic backends exist. It is deliberately gated behind --experimental and --reviewed and is not a production FEM solver, wall-design check, tunnel lining check, face-stability check, basal-heave check, seepage analysis, full 2D/3D coupled consolidation analysis, or design calculation.
