...
Current strong-beta scope: file discovery, AGS/PDF/image/GIS/CAD classification, context-aware CSV/XLSX schema inference for common borehole IDs and groundwater-depth tables, evidence references, canonical GroundModel construction, GroundModel coordinate maps with CRS/local-grid warnings, compact borehole strip logs, SPT-depth plots, lab-parameter depth charts, groundwater/monitoring summaries, standards-profile assumptions, verifier findings, detected branches, calculation readiness for bearing, settlement, evidence-prefilled experimental FEM foundation/excavation drafts, pile, liquefaction, and slope workflows, optional non-executing calculation input drafts, warnings, and recommended next steps. Analyze does not yet auto-run design calculations from the folder; it reports which deterministic workflow is ready, blocked, or needs explicit assumptions before routing. The optional agent swarm planner can consume this workspace evidence with --workspace --swarm.`,
  },
  {
    id: 'fem',
    title: 'geotech fem',
    content: `Experimental deterministic FEM case drafting and 3D FEM/WebGL previews. The current strong-beta surfaces prepare review-gated foundation-settlement and staged-excavation deformation analysis cases, then run built-in raft settlement and staged excavation deformation demos that validate FEM analysis cases, produce deterministic result manifests, and export self-contained WebGL artifacts without spending hosted-beta model calls.
...
Current scope: foundation-settlement and staged-excavation deformation drafts/demos only, linear elastic screening behavior, explicit draft inputs, workspace evidence prefill, fixed built-in demo geometry, deterministic result envelopes, validation warnings, and browser artifact export. \`geotech fem draft\` prepares editable \`analysis_case.json\` inputs without running a solver; demo commands execute only the built-in experimental previews. Workspace prefill can carry material stiffness, unit weight, groundwater assumptions, and evidence references into a draft, but geometry, loads, staging, mesh intent, and engineering approval still require explicit user review. The excavation preview adds staged field controls for surface settlement, horizontal displacement, and wall-deflection proxy. FEM manifests also include optional resultFields, steps, and datasets metadata so future shaft, tunnel, pile-group, embankment, seepage, and slope previews can share the same validation and viewer contract. It is deliberately gated behind --experimental and is not a production FEM solver, wall-design check, basal-heave check, seepage analysis, or design calculation.

Agent and swarm mode see FEM through deterministic routing tools rather than prompt-only claims. geotech fem agent is a narrower LLM brain for FEM planning: it may only list FEM capabilities, prepare review-gated case drafts, and validate FEM cases. With --workspace, it receives FEM-only GroundModel readiness context and the same evidence-prefill contract as geotech fem draft. It cannot run solvers, write WebGL artifacts, or invent FEM displacement/reaction values. In general swarm mode, simulation-stage FEM/calculation tool outputs are threaded into reviewer prompts as deterministic tool context so RiskReviewer validation does not depend on a model restating every case detail in prose. Blocked validation is returned as inspectable review data so reviewer agents can reject unsafe cases with specific blocker codes. Shaft deformation, tunnel volume-loss settlement, and pile-group interaction remain draft/planned routes until deterministic engines are added. The purpose is to stabilize the provider-neutral FEM case/result contract before future GroundModel-to-calculation routing, report-derived input drafting, and agent review workflows use the same deterministic boundary.`,
  },
  {
    id: 'ingest',
